...
```bash
service-1$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
service-1$ sam local start-api
service-1$ curl http://localhost:3000/
```
...
```bash
service-1$ sam logs -n HelloWorldFunction --stack-name service-1 --tail
```
...
```bash
aws cloudformation delete-stack --stack-name service-1
```
