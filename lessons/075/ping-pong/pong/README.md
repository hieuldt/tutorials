...
```bash
pong$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
pong$ sam local start-api
pong$ curl http://localhost:3000/
```
...
```bash
pong$ sam logs -n HelloWorldFunction --stack-name pong --tail
```
...
```bash
aws cloudformation delete-stack --stack-name pong
```
