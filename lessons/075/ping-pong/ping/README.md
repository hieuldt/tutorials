...
```bash
ping$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ping$ sam local start-api
ping$ curl http://localhost:3000/
```
...
```bash
ping$ sam logs -n HelloWorldFunction --stack-name ping --tail
```
...
```bash
aws cloudformation delete-stack --stack-name ping
```
